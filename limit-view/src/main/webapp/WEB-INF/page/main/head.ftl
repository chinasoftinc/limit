<div id="top" style="overflow:hidden">
	<div id="topIntro" style="margin:5px">
		<span id="user_operator">
			 <b>欢迎您 -&nbsp;&nbsp;${Session.user.nickname}  |${Session.user.username} </b>
					<a id="userMenu" href="javascript:void(0)"><img width="17px" height="17px" src="${ctx}/resource/img/setting.jpg"></a>
		</span>
		<br/>
	</div>
</div>
...
<script>
	...
	<#-- 个人信息 -->
	function userInfo(){
		$.createSimpleWindowAutoScroll("userInfoWin","个人信息", 600, 450, "${ctx}/user/info?operation=info");
	}
	
	<#-- 修改密码 -->
	function modifyPwd(){
		$.createSimpleWindowAutoScroll("userInfoWin","修改密码", 650, 280, "${ctx}/user/info?operation=modifyPwd");
	}
